...
/*   Updated: 2024/02/26 23:03:20 by gcros            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point	apply_proj(t_point point, t_mat4 *model_mat)
{
	...
	p.coord = fdf_mat4_x_vec3(*model_mat, point.coord);
	p.color = point.color;
...
int	get_proj_points(t_object *obj, t_object *proj_obj, t_projection *proj)
{
	...
	proj_obj->y = obj->y;
	mvp = get_ident_mat();
	mvp = fdf_mat4_mul(get_model_mat(proj), mvp);
	mvp = fdf_mat4_mul(get_view_mat(proj), mvp);
	mvp = fdf_mat4_mul(proj->mat_proj, mvp);
	i = 0;
	while (i < obj->y * obj->x)
	{
